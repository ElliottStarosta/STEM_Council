...
async function loadResourcesContent() {
  try {
    const settingsResponse = await fetch(
      "/src/content/resources-settings.json"
    );
    ...
    for (const filename of index.resources) {
      const resourceData = await ContentLoader.fetchMarkdown(
      ...
      if (resourceData && resourceData.frontmatter) {
        ...
        resourcesHTML += `
          <div class="resource-card" data-type="${resource.type}" data-markdown-file="resources/${filename}">
            <div class="card-icon">
